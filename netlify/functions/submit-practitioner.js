...
const CUMULATIVE_LOAD_LABELS = {
  isolated: 'Isolated engagement (no significant recent or upcoming demands)',
  minor_sequence: 'Part of a minor sequence (one or two other demands this week)',
  significant_sequence: 'Part of a significant sequence (multiple demands this week)',
  intensive_block: 'Part of an intensive block (concurrent assessments, training, or multiple sessions)',
  unknown: 'Unknown — not yet discussed with client'
};
...
// Evidence summary for email output
const EVIDENCE_SUMMARY = `
<div style="background: #f8f9fa; border: 1px solid #ddd; border-radius: 8px; padding: 20px; margin: 30px 0;">
  <h3 style="color: #1a1a2e; margin: 0 0 15px 0; font-size: 1.1rem;">
    📚 Evidence Base
  </h3>
  <p style="color: #555; font-size: 0.9rem; margin: 0 0 15px 0;">
  ...
  <div style="font-size: 0.85rem; color: #666; line-height: 1.6;">
    <p style="margin: 0 0 10px 0;"><strong>Cognitive Load & Learning:</strong> 
    Sweller (1988); Sweller et al. (2011); Baddeley (2012); Paas & van Merriënboer (2020)</p>
    
    <p style="margin: 0 0 10px 0;"><strong>Stress & Executive Function:</strong> 
    Shields et al. (2016); Arnsten (2009); McEwen & Morrison (2013)</p>
    
    <p style="margin: 0 0 10px 0;"><strong>Environment & Performance:</strong> 
    Evans & McCoy (1998); Ulrich et al. (2008)</p>
    
    <p style="margin: 0 0 10px 0;"><strong>Fatigue & Diminishing Returns:</strong> 
    Boksem & Tops (2008); Hockey (2013); Hill & Knox (2009)</p>
    
    <p style="margin: 0 0 10px 0;"><strong>Masking & Emotional Labour:</strong> 
    Hochschild (1983); Grandey (2000); Leary & Kowalski (1990)</p>
    
    <p style="margin: 0 0 10px 0;"><strong>Delayed & Cumulative Harm:</strong> 
    Maslach & Leiter (2016); Sonnentag & Fritz (2015); Wampold & Imel (2015)</p>
    
    <p style="margin: 0 0 10px 0;"><strong>Consent & Autonomy:</strong> 
    Beauchamp & Childress (2019); Deci & Ryan (2000)</p>
    
    <p style="margin: 0 0 0 0;"><strong>Assessment Validity:</strong> 
    Messick (1995); Brookhart (2013)</p>
  </div>
</div>
`;
...
function generateSummary(clientResponses, practitionerData, capacityBand) {
  ...
  if (practitionerData.cumulativeLoad === 'significant_sequence' || practitionerData.cumulativeLoad === 'intensive_block') {
    capacityWarnings.push(`⚠️ Cumulative load: ${CUMULATIVE_LOAD_LABELS[practitionerData.cumulativeLoad]}`);
    if (practitionerData.cumulativeLoadDetails) {
      capacityWarnings.push(`Details: ${practitionerData.cumulativeLoadDetails}`);
    }
  }
  ...
  return {
    ...
    cumulativeLoad: CUMULATIVE_LOAD_LABELS[practitionerData.cumulativeLoad] || practitionerData.cumulativeLoad,
    cumulativeLoadDetails: practitionerData.cumulativeLoadDetails || null,
  ...
  return `
    <div style="font-family: Georgia, serif; max-width: 700px; margin: 0 auto; padding: 20px;">
      ...
      <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px;">
        ...
        <tr>
          <td style="padding: 8px; border: 1px solid #ddd;"><strong>Cumulative Load</strong></td>
          <td style="padding: 8px; border: 1px solid #ddd;">${summary.cumulativeLoad}${summary.cumulativeLoadDetails ? ` — ${summary.cumulativeLoadDetails}` : ''}</td>
        </tr>
        <tr>
...
export async function handler(event) {
  ...
  try {
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // SERVER-SIDE VALIDATION (before any processing)
    // UI-only gates are not sufficient — must enforce here
    // ═══════════════════════════════════════════════════════════════════════════

    // 1. Validate band override value
    const validBandValues = ['no', 'green', 'amber', 'red'];
    if (!validBandValues.includes(practitionerData.bandOverride)) {
      return {
        statusCode: 400,
        body: JSON.stringify({ error: 'Invalid band override value' })
      };
    }

    // 2. If override is set, justification is required
    if (practitionerData.bandOverride !== 'no') {
      if (!practitionerData.overrideJustification || !practitionerData.overrideJustification.trim()) {
        return {
          statusCode: 400,
          body: JSON.stringify({ error: 'Override justification required when changing band' })
        };
      }
    }
    ...
    for (const field of requiredFields) {
      if (!practitionerData[field] || (typeof practitionerData[field] === 'string' && !practitionerData[field].trim())) {
        return {
          statusCode: 400,
    ...
    // 4. Responsibility acknowledgements (checkboxes)
    if (!practitionerData.acknowledgeHarm) {
      return {
        statusCode: 400,
        body: JSON.stringify({ error: 'Must acknowledge consideration of foreseeable harm' })
      };
    }
    
    if (!practitionerData.acknowledgeResponsibility) {
      return {
        statusCode: 400,
        body: JSON.stringify({ error: 'Must acknowledge responsibility for decision' })
      };
    }

    // 5. Determine effective band (may be overridden)
    const effectiveBand = practitionerData.bandOverride !== 'no' 
      ? practitionerData.bandOverride 
      : session.capacity_band;

    // 6. RED-specific enforcement
    if (effectiveBand === 'red') {
      // Cannot "proceed as planned" under RED
      if (practitionerData.decision === 'proceed') {
        return {
          statusCode: 400,
          body: JSON.stringify({ error: 'Cannot "proceed as planned" under RED conditions. Must modify, postpone, refer, or clarify.' })
        };
      }
      
      // Cannot claim full assessment validity under RED
      if (practitionerData.assessmentValidity === 'full') {
        return {
          statusCode: 400,
          body: JSON.stringify({ error: 'Cannot claim full assessment validity under RED conditions' })
        };
      }
      
      // Must acknowledge RED justification requirement
      if (!practitionerData.acknowledgeRedJustification) {
        return {
          statusCode: 400,
          body: JSON.stringify({ error: 'Must acknowledge RED justification requirement' })
        };
      }
    }

    // ═══════════════════════════════════════════════════════════════════════════
    // VALIDATION PASSED — Generate summary
    // ═══════════════════════════════════════════════════════════════════════════
    ...
    // Step 1: Update session as completed (but NOT results_sent_at yet)
    const { error: updateError } = await supabase
      ...
      .update({
        ...
        // Note: results_sent_at is set AFTER successful email send
      })
      .eq('practitioner_token', token);
    ...
    // Determine where to send results
    // results_email is the reviewer/practitioner copy destination
    // In self-use mode, this may be the same as client_email
    const resultsEmail = session.results_email || session.practitioner_email;

    // Step 2: Send emails to both parties
    const emailPromises = [
      // Send to client (always to client_email)
      resend.emails.send({
        from: fromEmail,
        reply_to: replyTo,
      ...
      // Send to practitioner/reviewer (to results_email)
      resend.emails.send({
        from: fromEmail,
        reply_to: replyTo,
        to: resultsEmail,
        subject: `Access Assessment Complete - ${session.client_name}`,
        html: `
          <div style="font-family: Georgia, serif; max-width: 600px; margin: 0 auto; padding: 20px;">
            <p style="font-size: 16px; line-height: 1.6;">
    ...
    // Step 3: Only AFTER successful email send, mark results_sent_at
    ...
    if (sentUpdateError) {
      console.error('Failed to update results_sent_at:', sentUpdateError);
      // Don't throw - emails were sent successfully, this is just timestamp
    }
